...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className='fixed top-0 left-0 w-full z-50'>
      <div className={`transition-all ease-in-out duration-500 ${isScrolled ? 'bg-white shadow-md' : 'bg-transparent'}`}>
...
<button onClick={() => setIsOpen(!isOpen)} className="z-50 py-4 absolute top-0.5 flex flex-col space-y-2">
    <span 
...
{isOpen && (
    <div className="fixed inset-0 bg-green-700 px-10 py-12 flex flex-col items-start h-full overflow-y-auto">
      
      <Link href="/" onClick={() => setIsOpen(!isOpen)} className="text-white hover:text-indigo-700 py-3">Main</Link>
      <Link href="/menu" onClick={() => setIsOpen(!isOpen)} className="text-white hover:text-indigo-700 pb-3">Menu</Link>
      <Link href="/about" onClick={() => setIsOpen(!isOpen)} className="text-white hover:text-indigo-700 pb-3">About us</Link>
      <Link href="/contact" onClick={() => setIsOpen(!isOpen)} className="text-white hover:text-indigo-700 pb-3">Contact me</Link>
